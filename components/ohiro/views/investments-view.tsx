    ...
    <div className="flex flex-col gap-6 p-4 md:p-6 max-w-[1400px] mx-auto">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="p-4 rounded-lg border border-border/40 bg-card/40">
          <div className="text-[10px] font-mono text-muted-foreground tracking-wider uppercase mb-1">USD Assets</div>
          <div className="text-xl font-mono font-bold text-foreground">$ {totalUSD.toFixed(2)}</div>
        </div>
        <div className="p-4 rounded-lg border border-border/40 bg-card/40">
          <div className="text-[10px] font-mono text-muted-foreground tracking-wider uppercase mb-1">Monthly Contribution</div>
          <div className="text-xl font-mono font-bold text-foreground">{formatCurrency(monthlyContribution)}</div>
        </div>
        <div className="p-4 rounded-lg border border-border/40 bg-card/40">
          <div className="text-[10px] font-mono text-muted-foreground tracking-wider uppercase mb-1">Assets</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className={cn("rounded-lg border border-border/40 bg-card/60 overflow-hidden", pieData.length > 0 ? "lg:col-span-2" : "lg:col-span-3")}>
          <div className="p-4 border-b border-border/40">
            <div className="text-[11px] font-mono text-muted-foreground/50 tracking-widest uppercase">Portfolio Assets</div>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-xs font-mono">
              <thead>
                <tr className="border-b border-border/30 bg-muted/10">
                  {["Asset", "Class", "Balance", "Currency", "Rate", "In USD", "Monthly", ""].map((h) => (
                    ...
                    <tr key={inv.id} className="border-b border-border/20 hover:bg-muted/20 transition-colors">
                      ...
                      <td className="px-3 py-2.5">
                        <span className="inline-flex items-center gap-1">
                          ...
                          <span className="text-muted-foreground">{INVESTMENT_CLASS_LABEL[inv.class]}</span>
                        </span>
                      </td>
                      <td className="px-3 py-2.5 text-foreground">{inv.currency !== "USD" ? `${inv.amount.toFixed(2)}` : formatCurrency(inv.amount)}</td>
                      <td className="px-3 py-2.5"><CurrencyBadge currency={inv.currency} /></td>
                      <td className="px-3 py-2.5 text-muted-foreground">{inv.currency !== "USD" ? inv.exchangeRate.toFixed(2) : "—"}</td>
